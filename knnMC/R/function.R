...
#'
#' Core functions for a Monte Carlo study that decomposes the mean squared error (MSE)
#' of k-nearest neighbors (k-NN) regression into bias and variance components under a
#' known data generating process (DGP).
#'
#' The DGP follows:
#' \deqn{Y = f(X) + \varepsilon, \quad X \sim \mathrm{Unif}(0,1)^d,\ \varepsilon \sim \mathcal{N}(0,\sigma^2).}
#'
#' This file is designed for the STA380 "Basic Implementation Check":
#' functions + roxygen2 documentation + unit tests + vignette.
#'
#' @name knn_mc_toolkit
#' @docType package
NULL
...
#'
#' Draws \eqn{X \sim \mathrm{Unif}(0,1)^d}. 3803803830830
#'
...
#'
#' Two target functions are supported:
#' \itemize{
#'   \item \code{"1d"}: \eqn{f(x) = \sin(2\pi x)}
#'   \item \code{"2d"}: \eqn{f(x_1,x_2) = \sin(\sqrt{x_1^2 + x_2^2})}
#' }
#'
#' @param x A numeric vector (for 1d) or a numeric matrix with 2 columns (for 2d).
...
#'
#' Creates \eqn{Y = f(X) + \varepsilon} with \eqn{\varepsilon \sim N(0,\sigma^2)}.
#'
...
#' @param seed Optional integer. If provided, sets the random seed before noise generation.
#'
...
#' where neighbors are defined by Euclidean distance.
#'
...
#'
#' @param grid_size Integer. For d=1, number of points. For d=2, points per axis (total grid_size^2).
...
#' Monte Carlo estimate of bias–variance decomposition for k-NN
#'
...
#' \itemize{
#'   \item \eqn{\mathrm{Bias}(x) = E[\hat f(x)] - f(x)}
...
#'
#' @return A list with the grid, true function values, and Monte Carlo estimates of bias, variance,
#' and MSE components at each grid point, plus a summary curve (integrated MSE over the grid).
#'
#' @examples
...
#'
#' Runs \code{mc_knn_decomp()} for each k in \code{k_values} and summarizes the integrated MSE over the grid.
#'
...
#'
#' Convenience plotting function for the 1d baseline case.
#'
#' @param res Output from \code{mc_knn_decomp()} with d=1.
